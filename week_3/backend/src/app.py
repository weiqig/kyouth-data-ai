    ...
    try:
        ...
        if pdf_data:
            if "skill gaps" in user_message:
                ...
                if gaps:
                    prompt += "Be brief and return the skill gaps in a list categorized accordingly. Do not explain.\n"
                    prompt += f"the skill gaps from the resume are as follows: {str(gaps.gaps)}\n"
                    prompt += "follow up by asking the user if they need further help on their resume.\n"
                else:
                    prompt += "An issue occurred when trying to find skill gaps, proceed with your own assumptions.\n"
                    prompt += (
                        f"[Context from uploaded file '{file_name}': {pdf_data}]\n\n"
                    )
            else:
        ...
        print(
            f"-> [METRICS] Model: {DEFAULT_MODEL} | Tokens: {output['tokens_used']} | Latency: {output['total_time']:.2f}ms"
        )
