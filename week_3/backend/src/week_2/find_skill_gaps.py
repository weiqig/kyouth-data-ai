...
def model_prompt(resume_content: str) -> str:
    return (
        f"Extract comma-separated list of technical skills, languages, and tools from resume."
    ...
    for attempt_num in range(RETRY_ATTEMPTS):
        try:
            ...
            with sqlite3.connect(str(db_url)) as conn:
                ...
                cursor = execute_sql(conn, BASE_DIR / "sql/get_tech_stack.sql")
            ...
            return SkillGapResult(
                gaps=gaps, tokens_used=tokens_used, total_time=total_time
            )
